...
export function Sidebar() {
  ...
  const isTicketsActive = location.pathname === '/tickets' || (location.pathname.startsWith('/tickets/') && !location.pathname.startsWith('/tickets/new'));
  ...
  return (
    <aside className="flex w-56 flex-col border-r border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
      <div className="flex h-14 items-center justify-between border-b border-slate-200 px-4 dark:border-slate-700">
        <span className="font-semibold text-slate-800 dark:text-slate-100">OpsDesk</span>
                    ...
                    {notifications.map((n) => (
                      ...
                      >
                        <span className="font-medium">
                          {n.type === 'message' ? 'New message' : 'Status changed'}
                        </span>
                        <span className="break-words text-xs text-slate-500 dark:text-slate-400">
                          Ticket · {new Date(n.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}
                        </span>
                      </button>
                    ))}
                  </div>
